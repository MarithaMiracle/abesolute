          ...
          html: `
            <div style="font-family:system-ui, sans-serif; color:#1f2937; line-height:1.6;">
              ...
              ${body['Attendance'] === 'yes' ? `
                ...
                <p style="font-weight:600; margin:12px 0 0;">${body['First Name']} ${body['Surname']} - Attending${body['Table no.'] || body['Seat'] ? `<br /><span style="font-size:14px; color:#4b5563;">${[body['Table no.'] ? `Your assigned table number is Table ${body['Table no.']}.` : '', body['Seat'] ? `Your assigned seat number is Seat ${body['Seat']}.` : ''].filter(Boolean).join('<br />')}</span>` : ''}</p>
